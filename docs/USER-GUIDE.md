...
3. 后端顺序：
   - 尽力创建系统还原点（失败不阻断）
   - 创建备份会话并备份所有选中项（**任一项备份失败则整次中止，不删任何东西**；PATH 残留写入会话 `path.json`）
...
4. 结果面板显示 deleted / failed / skipped，以及 backup 目录路径；可导出 **HTML 报告**。

备份位置：`%PROGRAMDATA%\Remova\Backup\<时间戳>_<应用名>\`

### 1.7 还原（需要时）

工具栏展开更多工具 → **还原**，可选择指定备份会话 → **确认还原**。
