...
fn tmp_root() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .map(|d| d.as_nanos())
        .unwrap_or(0);
    let p = std::env::temp_dir().join(format!("remova_pipeline_{}_{}", std::process::id(), nanos));
...
fn pipeline_dry_run_backup_delete_restore() {
    // Serialize with prune test: both mutate REMOVA_BACKUP_DIR (NEW-B).
    let _guard = crate::backup::lock_backup_env();
    let root = tmp_root();
    let bak = root.join("Backup");
    ...
    assert!(app_dir.join("bin").join("app.bin").exists(), "bin restored");
    assert_eq!(
