...
pub fn scan_store_apps() -> Vec<InstalledApp> {
    ...
    for pkg in packages {
        ...
        // Prefer AppxManifest DisplayName over package identity name (often a publisher hash).
        let display = read_manifest_display_name(&install_location)
            .filter(|s| {
                let t = s.trim();
                !t.is_empty()
                    && !t.starts_with("ms-resource:")
                    && !(t.len() >= 16
                        && t.chars()
...
/// Crude AppxManifest DisplayName extract (avoids full XML dep).
fn read_manifest_display_name(install_location: &str) -> Option<String> {
...
mod tests {
    ...
    fn manifest_display_name() {
        ...
        std::fs::write(
            &xml,
            r#"<?xml version="1.0"?><Package><Properties><DisplayName>计算器</DisplayName></Properties></Package>"#,
        )
        .unwrap();
        let name = super::read_manifest_display_name(&dir.to_string_lossy());
        assert_eq!(name.as_deref(), Some("计算器"));
