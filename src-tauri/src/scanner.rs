...
pub fn analyze_associations(
    ...
    scan_path_env(&name_slugs, &install_low, &mut items);
    scan_shell_extensions(&name_slugs, &install_low, &mut items);
    scan_drivers(&name_slugs, &install_low, &mut items);
    scan_other_drive_roots(&name_slugs, &mut items);
    scan_webview_masks(&name_slugs, &mut items);
...
/// User + Machine PATH entries that point at / contain the product (SOP §2⑤).
fn scan_path_env(name_slugs: &[String], install_low: &str, items: &mut Vec<CleanupItem>) {
    let name_norms: Vec<String> = name_slugs.iter().map(|s| normalize_for_match(s)).collect();
    let mut seen = std::collections::HashSet::new();
...
/// Shell extension / CLSID / ProgID leftovers (SOP §2④).
fn scan_shell_extensions(name_slugs: &[String], install_low: &str, items: &mut Vec<CleanupItem>) {
    let name_norms: Vec<String> = name_slugs.iter().map(|s| normalize_for_match(s)).collect();
...
/// Kernel/file-system drivers under Services (Type=1) (SOP §2①).
fn scan_drivers(name_slugs: &[String], install_low: &str, items: &mut Vec<CleanupItem>) {
...
/// Shallow cross-drive roots: `D:\.ollama`, `D:\App` (SOP 附录 A).
fn scan_other_drive_roots(name_slugs: &[String], items: &mut Vec<CleanupItem>) {
...
fn webview_mask_matches(dir_name: &str, name_slugs: &[String]) -> bool {
...
fn scan_webview_masks(name_slugs: &[String], items: &mut Vec<CleanupItem>) {
...
mod tests {
    ...
    fn webview_mask_requires_product_name() {
        ...
        assert!(super::webview_mask_matches("ollama.exe", &slugs));
        assert!(super::webview_mask_matches("Ollama.exe", &slugs));
        ...
        assert!(!super::webview_mask_matches("chrome.exe", &slugs));
        assert!(!super::webview_mask_matches("Code.exe", &slugs));
        assert!(!super::webview_mask_matches("EBWebView", &slugs));
        assert!(!super::webview_mask_matches("msedge.exe", &slugs));
        // Too-short stem is ignored.
        assert!(!super::webview_mask_matches("ab.exe", &slugs));
    }
}
