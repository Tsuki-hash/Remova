...
export function ManageListPage({
  ...
  const toggle = async (item: ManageItem) => {
    ...
    try {
      if (tab === "startup") {
        await api.setStartupEnabled(item.location, !item.enabled);
      } else if (tab === "services") {
        await api.setServiceStartDisabled(item.name, item.enabled);
      } else {
        await api.setTaskEnabled(item.name, !item.enabled);
      }
      toast.success(
        item.enabled
  ...
  const rows = useMemo(() => {
    ...
    return items.filter((it) => {
      if (onlyOn && !it.enabled) return false;
  ...
  return (
    <div style={{ ...css.card, flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      ...
      >
        ...
        <span style={css.chip}>
          {busy && items.length === 0 ? "…" : `${rows.length}${rows.length !== items.length ? ` / ${items.length}` : ""}`}
        </span>
        <input
        ...
        <button style={{ ...css.btnGhost, marginLeft: "auto" }} disabled={busy} onClick={() => void reload()}>
          {L.manageReload}
        </button>
      </div>
      <div
      ...
      <div ref={scrollRef} style={{ ...css.scroll, padding: "4px 8px", position: "relative" }}>
        {busy && items.length === 0 && (
          <div style={{ ...css.muted, padding: 16 }}>{L.loadingManage}</div>
        )}
        {!busy && rows.length === 0 && (
          <div style={{ ...css.muted, padding: 28, textAlign: "center" as const }}>{L.emptyList}</div>
        )}
        {rows.length > 0 && (
                ...
                >
                  <div
                    style={{
                      ...
                      height: 36,
                      borderRadius: 10,
                      background: it.enabled ? "var(--accent-soft)" : "var(--surface-2)",
